...
mod player_integration_tests {
    ...
    use combat_game::models::{
        player::{Player, m_Player, PlayerAssert}, 
        beast::{Beast, m_Beast}, 
        beast_stats::{BeastStats, m_BeastStats, BeastStatsActionTrait}, 
        beast_skill::{BeastSkill, m_BeastSkill}, 
        skill::{Skill, m_Skill}, 
    ...
    fn test_player_creation_and_initialization() {
        ...
        assert_eq!(player.creation_day, player.last_active_day, "Creation and last active should be same initially");
    }
    ...
    fn test_player_unique_addresses() {
        ...
        assert!(player1.current_beast_id != player2.current_beast_id, "Players should have different beast IDs");
    }
    ...
    fn test_player_activity_tracking() {
        ...
        assert_eq!(player.creation_day, expected_day.try_into().unwrap(), "Creation day should match");
        assert_eq!(player.last_active_day, expected_day.try_into().unwrap(), "Last active should match creation initially");
        
        // Simulate time passage and activity
        testing::set_block_timestamp(creation_timestamp + SECONDS_PER_DAY + 3600); // Next day + 1 hour
        ...
        assert_eq!(updated_player.creation_day, expected_day.try_into().unwrap(), "Creation day should not change");
        assert_eq!(updated_player.last_active_day, new_expected_day.try_into().unwrap(), "Last active should update");
        assert!(updated_player.last_active_day > updated_player.creation_day, "Last active should be after creation");
    }
    ...
    fn test_multiple_activity_updates() {
        ...
        assert_eq!(same_day_player.last_active_day, initial_last_active, "Same day activities should not change day");
        ...
        assert!(different_day_player.last_active_day > initial_last_active, "Different day activity should update last_active_day");
    }
    ...
    fn test_player_state_consistency_across_operations() {
        ...
        assert_eq!(after_win.creation_day, initial_creation_day, "Creation day should never change");
        assert_eq!(after_win.battles_won, 1, "Wins should increment");
        
        player_system.update_profile(false, beast_id);
        let after_loss = store.read_player_from_address(PLAYER1());
        assert_eq!(after_loss.creation_day, initial_creation_day, "Creation day should never change");
        ...
        assert_eq!(after_beast_change.creation_day, initial_creation_day, "Creation day should never change");
    ...
    fn test_player_zero_state_validation() {
        ...
        assert_eq!(nonexistent_player.address, constants::ZERO_ADDRESS(), "Zero player should have zero address");
        ...
        assert_eq!(nonexistent_player.last_active_day, 0, "Zero player should have zero last active");
