...
impl IntoStatusConditionFelt252 of Into<StatusCondition, felt252> {
    fn into(self: StatusCondition) -> felt252 {
        match self {
            ...
            StatusCondition::Weakness => 8,
            StatusCondition::Stunned => 9,
        }
    }
}
...
impl IntoU8StatusCondition of Into<u8, StatusCondition> {
    fn into(self: u8) -> StatusCondition {
        match self {
            ...
            8 => StatusCondition::Weakness,
            9 => StatusCondition::Stunned,
...
mod tests {
    ...
    fn test_status_condition_to_u8() {
        ...
        let weakness = StatusCondition::Weakness;
        let stunned = StatusCondition::Stunned;
        ...
        assert_eq!(weakness.into(), 8_u8, "Cursed should convert to 8");
        assert_eq!(stunned.into(), 9_u8, "Cursed should convert to 9");
    }
    ...
    fn test_status_condition_to_felt252() {
        ...
        assert_eq!(poisoned.into(), 1_felt252, "Poisoned should convert to 1_felt252");
        assert_eq!(paralyzed.into(), 2_felt252, "Paralyzed should convert to 2_felt252");
        assert_eq!(asleep.into(), 3_felt252, "Asleep should convert to 3_felt252");
        assert_eq!(confused.into(), 4_felt252, "Confused should convert to 4_felt252");
        ...
        assert_eq!(weakness.into(), 8_felt252, "Cursed should convert to 8_felt252");
        assert_eq!(stunned.into(), 9_felt252, "Cursed should convert to 9_felt252");
    }
    ...
    fn test_u8_to_status_condition_valid() {
        ...
        assert_eq!(one.into(), StatusCondition::Poisoned, "1 should convert to Poisoned");
        assert_eq!(two.into(), StatusCondition::Paralyzed, "2 should convert to Paralyzed");
        assert_eq!(three.into(), StatusCondition::Asleep, "3 should convert to Asleep");
        assert_eq!(four.into(), StatusCondition::Confused, "4 should convert to Confused");
        assert_eq!(five.into(), StatusCondition::Burned, "5 should convert to Burned");
        assert_eq!(six.into(), StatusCondition::Frozen, "6 should convert to Frozen");
        assert_eq!(seven.into(), StatusCondition::Cursed, "7 should convert to Cursed");
        assert_eq!(eight.into(), StatusCondition::Weakness, "8 should convert to Weakness");
        assert_eq!(nine.into(), StatusCondition::Stunned, "9 should convert to Stunned");
    }
    ...
    fn test_u8_to_status_condition_invalid() {
        ...
        assert_eq!(ten.into(), StatusCondition::None, "10 should default to None");
        assert_eq!(hundred.into(), StatusCondition::None, "100 should default to None");
        assert_eq!(two_fifty_five.into(), StatusCondition::None, "255 should default to None");
        assert_eq!(two_hundred.into(), StatusCondition::None, "200 should default to None");
    }
}
